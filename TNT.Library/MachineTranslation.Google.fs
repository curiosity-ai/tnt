...
let internal createBatches (maxCodePoints: int) (strings: string list) : (string list list) =
    ...
    let countCodePoints (strings: string seq) : uint64 seq =
        (0UL, strings) ||> Seq.scan ^ fun cnt str -> cnt + uint64 str.Length
    ...
    let splitAtMaxCodePointsReached (strings: string list) : string list * string list =
        ...
        strings |> List.splitAt nextChunkLength
    ...
    let rec genBatches (chunks: string list list) (pending: string list) : string list list =
        ...
        | chunk, pending -> genBatches (chunk :: chunks) pending
...
let Translator =
    { ProviderName = "Google"
      Translate =
        fun (sourceLanguage, targetLanguage) strings ->
            ...
            let tryUse (tag: LanguageTag) : LanguageTag =
                ...
                | _ -> failwithf "Unsupported language %s. Supported are: [%s]" tag.Formatted (supportedTargetLanguageTags |> Seq.map string |> String.concat ",")

            let sourceLanguage, targetLanguage = tryUse sourceLanguage, tryUse targetLanguage

            let tryTranslate (strings: string list) : Result<(string * string) list, exn> =
                let request =
                    TranslateTextRequest(Parent = parent, SourceLanguageCode = string sourceLanguage, TargetLanguageCode = string targetLanguage, MimeType = "text/plain")

                request.Contents.AddRange(strings)

                let response =
                    try
                        Ok ^ client.TranslateText(request)
                    with e ->
                        Error e
                ...
                | Ok response ->
                    ...
                    if translations.Count <> strings.Length then
                        Error
                        ^ exn
                        ^ sprintf "Expected %d translations to be returned, received %d instead." strings.Length translations.Count
                    else
                        translations
                        |> Seq.mapi ^ fun i result -> strings.[i], result.TranslatedText
                        |> Seq.toList
                        |> Ok

            let rec translateBatches (results: (string * string) list list) (batches: string list list) : (string * string) list list * exn option =
                match batches with
                | [] -> results, None
                | batch :: batches ->
                    match tryTranslate batch with
                    | Error e -> results, Some e
                    | Ok result -> translateBatches (result :: results) batches

            strings
            |> createBatches RecommendedMaxCodePointsPerRequest
            |> translateBatches []
            |> fun (results, e) -> Seq.rev results |> Seq.collect id |> Seq.toList, e }
