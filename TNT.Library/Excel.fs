...
module TargetState =
    ...
    let parse =
        ...
        | "Final" -> Final
        | state -> failwithf "invalid state: '%s'" state

[<AutoOpen>]
module private Private =

    let ValidExcelStates = [ New; NeedsReview; Final ]

    let StateValidationList =

        let inline quote str = "\"" + str + "\""

        ValidExcelStates |> Seq.map TargetState.toString |> String.concat "," |> quote

    let sheetPostfix (stateName: string) = " - " + stateName
...
[<Literal>]
let ChangedPostfix = " (changed)"

[<Literal>]
let CurrentColumn = 3

[<Literal>]
let CurrentPostfix = " (current)"

[<Literal>]
let StateColumn = 4

[<Literal>]
let ContextsColumn = 5
...
let parse (Excel excel) : File<ImportUnit> =
    ...
    let tryGetProjectName (worksheetName: string) : ProjectName option =
        ...
        |> Seq.tryPick
           ^ fun stateName ->
               ...
               if worksheetName.endsWith postfix then
                   Some
                   ^ ProjectName worksheetName.[0 .. worksheetName.Length - postfix.Length - 1]
               else
                   None
    ...
    let (|EndsWith|_|) (postfix: string) (str: string) =
        if str.endsWith postfix then
            Some(str.[0 .. str.Length - postfix.Length - 1])
        else
            None
    ...
    let extractUnits (ws: IXLWorksheet) : ImportUnit list =
        ...
        let tryParseState (cell: IXLCell) : TargetState option =
            ...
            | true, NotEmptyTrimmed trimmed -> Some ^ TargetState.parse trimmed
            | _ -> None
...
let Exporter: Exporter =
    ...
    { Extensions = [ ".xlsx" ]
      ...
      DefaultFilename = fun projectName languageTag -> Filename ^ (string projectName) + "-" + (string languageTag) + defaultExtension
      SaveToPath =
        fun path file ->
            let (Excel bytes) = generate file
            File.saveBinary bytes path
      LoadFromPath = fun path -> [ File.loadBinary path |> Excel |> parse ]
