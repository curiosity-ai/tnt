...
module Exporters =
    ...
    let tryResolveExporterFromFilename (Filename filename) (exporters: Exporter list) : Exporter option =
        exporters
        |> Seq.tryFind
           ^ fun exporter -> exporter.Extensions |> Seq.exists ^ fun ext -> filename.endsWith ext

    let allDefaultFilenames (project: ProjectName) (language: LanguageTag) (exporters: Exporter list) : (Exporter * Export filename) list =
        exporters
        |> List.map ^ fun exporter -> exporter, exporter.DefaultFilename project language
