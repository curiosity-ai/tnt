...
type OutputComputationBuilder() =
    ...
    member inline __.YieldFrom(o: Output seq) : unit output = fun writer -> o |> Seq.iter writer

    member inline __.Yield(o: Output) : unit output = fun writer -> writer o
    ...
    member inline this.Delay(f: unit -> 'r output) = this.Bind(this.Return(), f)
    ...
    member this.While(predicate, body: unit -> unit output) : unit output =
        if predicate () then
            this.Bind(body (), fun () -> this.While(predicate, body))
        else
            this.Zero()
...
module Output =
    ...
    let sequence (output: 'r output list) : 'r list output =
        fun writer -> output |> List.map (run writer)

    let map (f: 'a -> 'b) (input: 'a output) : 'b output = fun writer -> input |> run writer |> f

    let ignore o = o |> map ignore

    let bind (f: 'a -> 'b output) (input: 'a output) : 'b output = output.Bind(input, f)
