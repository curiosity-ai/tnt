...
[<Struct>]
type IndentedString = IndentedString of level: int * string
...
module Format =
    ...
    let group name list = Group(name, list)

    let indented level str = IndentedString(level, str)
...
module IndentedStrings =
    ...
    let indent (strings: IndentedString list) =
        strings |> List.map ^ IndentedString.indent

    let strings (indent: Indent) (strings: IndentedString seq) =
        strings |> Seq.map ^ IndentedString.string indent |> Seq.toList

module Property =

    let rec indentedStrings =
        function
        | Property(name, value) -> [ IndentedString(0, name + ": " + value) ]
        | Group(name, list) ->
            [ yield IndentedString(0, name + ":")
              for property in list do
                  yield! indentedStrings property |> IndentedStrings.indent ]

    let strings indent property =
        property |> indentedStrings |> IndentedStrings.strings indent

module Properties =

    let indentedStrings properties =
        properties |> List.collect Property.indentedStrings

    let strings indent properties =
        properties |> List.collect ^ Property.strings indent
