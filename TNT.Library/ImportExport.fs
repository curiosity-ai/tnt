...
let export (project: ProjectName) (sourceLanguage: LanguageTag) (translation: Translation) : File<ExportUnit> =
    ...
    let toUnit (record: TranslationRecord) =
        ...
        |> function
            ...
            | TranslatedString.NeedsReview str -> Some(NeedsReview, str, Verification.verifyRecord record)
            | TranslatedString.Final str -> Some(Final, str, [])
            | TranslatedString.Unused _ -> None
        |> Option.map
           ^ fun (state, str, warnings) ->
...
module internal ImportHelper =
    ...
    let importNotes (notes: string list) : string list =
        notes |> Seq.map ^ Text.trim |> Seq.filter ^ (<>) "" |> Seq.toList
    ...
    module Text =
        let sentences (str: string) : string seq =
            ...
            r
            |> Seq.mapi ^ fun index str -> if index <> r.Length - 1 then str + "." else str
...
type ImportWarning =
    ...
    override this.ToString() =
        match this with
        | ProjectMismatch(wrong, expected) -> sprintf "%s: unexpected project name of <file>, expect %s." wrong.Formatted expected.Formatted
        | DuplicateImports language -> sprintf "%s found two or more files with the same language" language.Formatted
        | TranslationNotFound language -> sprintf "%s translation missing" language.Formatted
        | OriginalStringNotFound(language, tu) -> sprintf "%s original string not found: '%s'" language.Formatted (limitText tu.Source)
        | UnusedTranslationChanged(language, (before, _)) -> sprintf "%s unused translation changed: '%s'" language.Formatted (limitText before.Original)
        | IgnoredNewWithTranslation(language, (record, _)) -> sprintf "%s ignored translation of a record marked new: '%s'" language.Formatted (limitText record.Original)
        | IgnoredNewReset(language, (record, _)) -> sprintf "%s ignored translation to state new, even though it wasn't new anymore: '%s'" language.Formatted (limitText record.Original)

/// Import a number of translations and return the translations that changed.
let import (project: ProjectName) (translations: Translation list) (files: File<ImportUnit> list) : Translation list * ImportWarning list =
    ...
    if mismatchedNames <> [] then
        [], mismatchedNames |> List.map ^ fun wrong -> ProjectMismatch(wrong, project)
    else
        ...
        if duplicatedLanguages <> [] then
            [], duplicatedLanguages |> List.map DuplicateImports
        else
            ...
            let tryImportFile (file: File<ImportUnit>) : Translation option * ImportWarning list =
                ...
                | Some translation ->
                    ...
                    let updateRecord (pending: Map<string, ImportUnit>) (record: TranslationRecord) : (TranslationRecord * ImportWarning option) * Map<string, ImportUnit> =
                        ...
                        | Some unit ->
                            ...
                            let record, warningOpt =
                                ...
                                | New when unit.Target <> "" -> record, Some ^ IgnoredNewWithTranslation(language, (record, unit))
                                | New when record.Translated = TranslatedString.New -> record, None
                                | New -> record, Some ^ IgnoredNewReset(language, (record, unit))
                                | NeedsReview -> update ^ TranslatedString.NeedsReview unit.Target
                                | Translated
                                | Final -> update ^ TranslatedString.Final unit.Target

                            let record =
                                match unit.Notes with
                                | Some notes ->
                                    { record with
                                        Notes = importNotes notes }
                                | None -> record
                    ...
                    let newRecordsAndWarnings, unprocessed =
                        (updates, records) ||> List.mapFold updateRecord

                    let newRecords, warnings =
                        newRecordsAndWarnings |> List.unzip |> Snd.map ^ List.choose id
