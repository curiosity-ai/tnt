...
type TranslationReport =
    ...
    override this.ToString() =
        ...
        let details =
            match () with
            | _ when this.All = this.Translated -> sprintf "%d translated" this.Translated
            | _ when this.Unusable = 0 -> sprintf "%d of %d translated" this.Translated this.All
...
type TranslationResult =
    ...
    override this.ToString() =
        match this with
        | NoStringsToTranslate(translation) -> sprintf "No strings to translate for %s" (Translation.shortStatus translation)
        | TranslationUnchanged(report, translation) -> sprintf "%O %s (no changes)" report (Translation.shortStatus translation)
        | Translated(report, translation) -> sprintf "%O %s" report (Translation.shortStatus translation)
...
module Translate =
    ...
    let newStrings (translator: Translator) (sourceLanguage: LanguageTag) (translation: Translation) : TranslationResult * exn option =
        let toTranslate =
            translation.Records
                ...
                ||> List.mapFold
                    ^ fun pairs record ->
                        if record.Translated = TranslatedString.New && pairs.ContainsKey record.Original then
                            { record with
                                Translated = TranslatedString.NeedsReview pairs.[record.Original] },
