...
module private FileSystemHelpers =
    ...
    let isValidPathCharacter =
        ...
        fun c -> invalidPathChars |> Array.contains c |> not

    let (|ValidPath|_|) (path: string) =
        if not ^ isEmpty path && isTrimmed path && path |> Seq.forall isValidPathCharacter then
            Some path
        else
            None
    ...
    let isValidNameCharacter =
        ...
        fun c -> invalidNameChars |> Array.contains c |> not

    let (|ValidFilename|_|) (name: string) =
        if not ^ isEmpty name && isTrimmed name && name |> Seq.forall isValidNameCharacter then
            Some name
        else
            None
...
module RPath =
    ...
    let map (f: string -> string) (Path path) = path |> f |> parse

    let extend (right: 'tag rpath) (left: _ rpath) : 'tag rpath =
        Path.Combine(string left, string right) |> parse

    let inline ofFilename (fn: 'tag filename) : 'tag rpath = string fn |> parse

    let inline extendF (right: 'tag filename) (path: _ rpath) : 'tag rpath = path |> extend (ofFilename right)

    let inline at (absolute: Path) (path: _ rpath) : Path = absolute |> Path.extend (string path)
    ...
    let parent (path: 'tag rpath) : _ rpath option =
        ...
        if parentPath = null || parentPath = "" then
            ...
            Some ^ parse parentPath

    let name (path: 'tag rpath) : string = Path.GetFileName(string path)
    ...
    let ofParts (parts: string list) : 'tag rpath =
        Path.Combine(List.toArray parts) |> parse
...
module ARPath =
    ...
    let tryParse (path: string) =
        ...
        | ValidPath path ->
            ...
            ^ if Path.IsPathRooted(path) then
                  AbsolutePath ^ Path.parse path
              else
                  RelativePath ^ RPath.parse path
        | _ -> None
    ...
    let extend (right: 'tag arpath) (left: _ arpath) : 'tag arpath =
        ...
        | RelativePath(RPath "."), right -> right
        | left, RelativePath(RPath ".") -> left
        | _, AbsolutePath path -> AbsolutePath path
        | AbsolutePath path, RelativePath rel -> AbsolutePath(path |> Path.extend (string rel))
        | RelativePath(RPath parent), RelativePath(RPath rel) -> RelativePath ^ RPath.parse ^ normalize ^ Path.Combine(parent, rel)

module Path =

    let inline extend (right: 'tag rpath) (left: Path) : Path = left |> Path.extend (string right)

    let inline extendF (right: 'tag filename) (left: Path) : Path = left |> extend (RPath.ofFilename right)
...
module Filename =
    ...
    let inline toPath (fn: 'tag filename) : 'tag rpath = RPath.ofFilename fn

    let inline at (path: Path) (fn: 'tag filename) : Path = path |> Path.extend (toPath fn)

    let inline ofARPath (path: 'tag arpath) : 'tag filename = Path.GetFileName(string path) |> parse
...
module Selector =
    ...
    let select (existing: 'a list) (selector: 'a selector) =
        existing |> List.partition ^ fun item -> isSelected item selector

module ExternalCommand =

    let run command =
        let si = new ProcessStartInfo(FileName = "cmd.exe", Arguments = "/c " + command)
    ...
    let openFile (file: Path) =
        let si =
            new ProcessStartInfo(string file, UseShellExecute = true, CreateNoWindow = true)
