...
type ProjectName =
    ...
    member this.Formatted = quoted ^ string this
...
type LanguageTag =
    ...
    member this.Primary =
        string this |> (fun str -> str.Split('-')) |> Array.head |> LanguageTag

    member this.Formatted = string this |> sprintf "[%s]"
...
type LogicalContext =
    ...
    override this.ToString() =
        this
        |> function
            | LogicalContext context -> context
...
[<Struct>]
type OriginalStrings = private OriginalStrings of strings: Map<string, Set<LogicalContext>>
...
module OriginalStrings =
    ...
    let merge sequence =
        sequence |> Seq.collect strings |> create

/// A source of strings.
type Source = AssemblySource of AssemblySource rpath
