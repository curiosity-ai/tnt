...
type 't handle =
    ...
    member this.Instance = this._Instance |> fst
    member this.Destructor = this._Instance |> snd
...
let liftDisposable (instance: 't :> IDisposable) =
...
let map (f: 't -> 'i) (handle: 't handle) : 'i handle =
    ...
    (f handle.Instance, handle.Destructor) |> lift
