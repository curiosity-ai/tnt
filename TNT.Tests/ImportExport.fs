...
let ``wrong project`` () =
    ...
    |> snd
    |> should equal [ ProjectMismatch(project, expectedProject) ]
...
let ``files with same key are rejected`` () =
    ...
    import [] [ fileA; fileA ]
    |> snd
    |> should equal [ DuplicateImports(lang "de") ]
...
let ``files with different keys are processed`` () =
    ...
    import [] [ fileA; fileB ]
    |> snd
    |> should equal [ TranslationNotFound(lang "de"); TranslationNotFound(lang "us") ]
...
let ``translation is not returned if nothing changes`` () =
    ...
    let file = file "de" [ itu "source" "target" NeedsReview ]
...
let ``trimToMaxCharacters`` (a: string) (b: string) =
    a |> Text.trimToMaxCharacters 5 "..." |> should equal b
...
let ``trimming to a length less than the ellipsis' length`` (max: int) (expected: string) =
    ...
    "Hello" |> Text.trimToMaxCharacters max ".." |> should equal expected
