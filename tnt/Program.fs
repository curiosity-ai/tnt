...
let dispatch (command: obj) =
    ...
    let checkUnprocessed unprocessed =
        if not ^ Seq.isEmpty unprocessed then
            failwithf "found one ore more unprocessed arguments: %s"
            ^ String.concat "," unprocessed
    ...
    | :? AddOptions as opts ->
        ...
        | None, None
        | Some _, Some _ -> failwith "use either --language or --assembly to specify what should be added"
        | Some language, _ -> API.addLanguage language
        | _, Some assembly -> API.addAssembly assembly
    ...
    | :? RemoveOptions as opts ->
        ...
        match assembly with
        | None -> failwith "use --assembly to specify which assembly should be removed"
        | Some assembly -> API.removeAssembly assembly
    ...
    | :? ExportOptions as opts ->
        ...
        let exportPath = opts.To |> Option.ofObj |> Option.defaultValue "." |> ARPath.parse
    ...
    | :? ImportOptions as opts ->
        ...
        let importDirectory =
            let relativeDirectory =
                opts.From |> Option.ofObj |> Option.defaultValue "." |> RPath.parse

            Directory.current () |> Path.extend relativeDirectory
        ...
        let files =
            if opts.All then
                ...
                |> Seq.distinctBy ^ snd
                |> Seq.map ^ fun (exporter, fn) -> exporter, importDirectory |> Path.extendF fn
                |> Seq.toList
            else
                ...
                let resolveLanguage (language: string) =
                    ...
                    let potentialFilenames = Exporters.allDefaultFilenames project language AllExporters
                    ...
                    | [] ->
                        failwithf
                            "Found no file for language %s, expected one of '%s'."
                            language.Formatted
                            (potentialFilenames
                             |> Seq.map (string << snd)
                             |> Seq.distinct
                             |> String.concat ",")
                    | [ one ] -> one
                    | _ -> failwithf "For more than one file for language %s found." language.Formatted
                ...
                let resolveFileOrLanguage (filePathOrLanguage: string) =
                    ...
                    | None -> resolveLanguage filePathOrLanguage
                ...
                | filenamesOrLanguages, languages ->
                    ...
                    let viaArguments = filenamesOrLanguages |> List.map resolveFileOrLanguage
                    let viaOptions = languages |> List.map resolveLanguage
    ...
    | :? TranslateOptions as opts ->
        ...
        let selector =
            if opts.All then
                ...
                opts.Languages |> Seq.map resolveLanguage |> Seq.toList |> Select
    ...
    | :? ShowOptions as opts ->
        ...
        let selector =
            let languages = opts.Languages |> Seq.toList

            if languages = [] then
                SelectAll
            else
                languages |> Seq.map resolveLanguage |> Seq.toList |> Select

        API.show selector (opts.Details |> Seq.toList)

    | :? AddCommandOptions as opts -> API.addCommand opts.When opts.Command

    | :? RemoveCommandsOptions as opts -> API.removeCommands opts.When

    | :? ListCommandsOptions -> API.listCommands ()

    | :? EditCommandsOptions -> API.editCommands ()
...
let protectedMain args =
    ...
    | :? CommandLine.NotParsed<obj> as np ->
        if np.Errors.IsHelp() || np.Errors.IsVersion() then
            ok
        else
            failed
    | x -> failwithf "internal error: %A" x
